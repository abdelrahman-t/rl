...
class VelocityModel:
    ...
    def initialize(self, initialState):
        ...
        linearVelocity = transformToBodyFrame(getAverageLinearVelocity(s1.position, s0.position, self.frequency),
                                              s1.orientation)
    ...
    def updateState(self, state, action):
        ...
        position = integratePosition(state.position,
                                     transformToEarthFrame(linearVelocity, eulerToQuaternion(*orientation)),
                                     self.frequency)
